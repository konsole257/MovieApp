    ...
    {homeFeeds.map((homeFeed) => {
      ...
      return (
        <div key={homeFeed.id} className={`feed-div ${isPlayable ? 'playable' : ''}`}> 
          <motion.div layoutId={`feed-${homeFeed.id}`} transition={{ duration: .3 }} className="motion-div">
            <NavLink className="feed-link" state={{type: 'movie', trailer: homeFeed}} to={`${isPlayable ? '/Trailer/':`/Detail/`}${homeFeed.id}`} onClick={handleNavClick}>
          ...
          <div className="feed-info">
            <NavLink state={{type: 'movie'}} to={`/Detail/${homeFeed.id}`} onClick={handleNavClick}>
